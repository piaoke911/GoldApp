from kivy.app import App
from kivy.uix.label import Label
from kivy.uix.boxlayout import BoxLayout
from kivy.clock import Clock
import alpha_engine # 引入你的算法零件

class GoldUI(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.display = Label(text="📊 黄金量化指挥部\n正在加载引擎...", font_size='20sp', halign='center')
        self.add_widget(self.display)
        Clock.schedule_interval(self.refresh, 10) # 每10秒刷新

    def refresh(self, dt):
        # 模拟数据输入
        test_data = {"tips": 1.85, "us10y": 4.1, "dxy": 101.5, "gold": 2050.0}
        try:
            # 调用你 alpha_engine.py 里的逻辑
            res, reasons, score, details = alpha_engine.generate_signal_v2(test_data, [])
            self.display.text = f"【决策信号】\n{res}\n\n当前评分: {score}\n逻辑: {reasons[0] if reasons else '平稳'}"
        except Exception as e:
            self.display.text = f"故障: {str(e)}"

class GoldApp(App):
    def build(self):
        return GoldUI()

if __name__ == '__main__':
    GoldApp().run()
